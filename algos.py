...
def Qtesting1_comm_aware(s, communities):
    ...
    for community in communities:
        community_indices = np.array(community).astype(int)
        sample_size = min(5, len(community_indices))
        representative_sample = np.random.choice(community_indices, sample_size, replace=False)
        ...
        if initial_infected_count > 0:
            community_tests = len(community_indices)
            num_tests += community_tests
            stages = max(stages, 2)
        else:
            community_group_test = test_T1(s[community_indices])
            num_tests += 1
            stages = max(stages, 2)
...
def Qtesting2_comm_aware(s, communities):
    ...
    for community in communities:
        try:
            # Convert community indices to integers and print them for debugging
            community_indices = np.array(community).astype(int)
            print(f"Testing community with indices: {community_indices}")
            initial_tests = 1
            initial_stages = 1
            
            # Check if the indices are within bounds of the array s
            if not np.all(community_indices < len(s)):
                print(f"Index out of bounds for community: {community_indices}")
                continue

            # Perform the test and print the result
            number = test_T2(s[community_indices])
            print(f"Initial test result for community {community_indices}: {number}")

            if number == 4:
                break
            elif number in [3, 2, 1]:
                community_tests, community_stages = Qtesting2(s[community_indices])
                num_tests += community_tests
                stages += community_stages
            
            num_tests += initial_tests
            stages += initial_stages

        except IndexError as e:
            print(f"IndexError: {e} for community: {community_indices}")
            raise e
